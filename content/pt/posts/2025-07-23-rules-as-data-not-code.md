...
author: helio
layout: post
date: 2025-07-23T10:00:00+00:00
series:
...
Cada uma dessas é recuperável. Recuperam-se *construindo o loader como peça de engenharia*, não como uma chamada de `yaml.Unmarshal` enfiada no startup do serviço. O trabalho do resto desse post é descrever esse loader.
...
Algumas coisas nesse formato parecem pequenas e ganham o lugar.

O `version: 1` no topo do arquivo é o primeiro ponto de decisão do loader. No dia em que o schema mudar — e vai — o loader precisa saber qual schema aplicar. Versionar o formato do arquivo é o que permite o time adicionar campo sem quebrar carga histórica.
...
Eis o formato de um loader que ganha o nome. É mais do que um `yaml.Unmarshal`:
...
Aqui também é onde o valor monetário que entrou como string é rejeitado. O validador diz que `markup_percentage` tem que ser número; o loader retorna erro apontando linha 17, arquivo `de_markup.yaml`, índice 2. O operador recebe uma mensagem com a qual pode agir. O engine nunca vê a regra ruim.
...
O passo de compile é onde o entendimento de schema do loader encontra o entendimento de runtime do engine. Separar dele a validação é o que permite o loader produzir erros úteis na camada certa: erro de validação fala de YAML, erro de compile fala do engine.
