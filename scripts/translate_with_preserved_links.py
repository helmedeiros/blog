...
def extract_and_preserve_links(text):
    ...
    # 5. Preserve HTML formatting tags (like <u>, <strong>, etc.)
    html_format_pattern = r'<(/?)([a-zA-Z][a-zA-Z0-9]*)\b[^>]*>'
    html_formats = re.findall(html_format_pattern, text)
    for match in html_formats:
        full_match = f"<{match[0]}{match[1]}>"
        if full_match not in text:
            # Find the actual full tag
            if match[0]:  # closing tag
                actual_pattern = f"</{match[1]}>"
            else:  # opening tag
                actual_pattern = f"<{match[1]}[^>]*>"
            actual_matches = re.findall(actual_pattern, text)
            for actual_match in actual_matches:
                placeholder = f"__HTML_TAG_{counter}__"
                preserved_elements[placeholder] = actual_match
                text = text.replace(actual_match, placeholder, 1)
                counter += 1
...
def translate_with_ollama(text, model='llama3:latest'):
    ...
    prompt = f"""You are a professional translator. Translate the following Brazilian Portuguese text to English while preserving ALL placeholders exactly as they appear.

Rules:
- Keep placeholders like __HTML_LINK_0__, __MD_REF_1__, __IMG_REF_2__ exactly as written
- Translate only the Portuguese text, not the placeholders
